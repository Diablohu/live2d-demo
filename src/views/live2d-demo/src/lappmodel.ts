...
export class LAppModel extends CubismUserModel {
     ...
     * @param fileName
     */
    public loadAssets(dir: string, fileName: string): Promise<LAppModel> {
        ...
        return fetch(`${this._modelHomeDir}${fileName}`)
            ...
            .then((arrayBuffer) => {
                ...
                // 結果を保存
                return this.setupModel(setting);
            })
            .catch((error) => {
                // model3.json読み込みでエラーが発生した時点で描画は不可能なので、setupせずエラーをcatchして何もしない
                CubismLogError(
                    `Failed to load file ${this._modelHomeDir}${fileName}`,
                );
                throw error;
            });
    }
     ...
     */
    private setupModel(setting: ICubismModelSetting): Promise<LAppModel> {
        return new Promise((resolve) => {
            ...
            const loadCubismExpression = (): void => {
                ...
                if (this._modelSetting.getExpressionCount() > 0) {
                    const count: number =
                        this._modelSetting.getExpressionCount();

                    for (let i = 0; i < count; i++) {
                        const expressionName =
                            this._modelSetting.getExpressionName(i);
            ...
            const loadCubismPhysics = (): void => {
                ...
                if (this._modelSetting.getPhysicsFileName() !== '') {
                    const physicsFileName =
                        this._modelSetting.getPhysicsFileName();
            ...
            const setupBreath = (): void => {
                ...
                const breathParameters: csmVector<BreathParameterData> =
                    new csmVector();
            ...
            const loadCubismMotion = (): void => {
                ...
                const motionGroupCount: number =
                    this._modelSetting.getMotionGroupCount();
                ...
                // モーションの読み込み
                for (let i = 0; i < motionGroupCount; i++) {
                    this.preLoadMotionGroup(group[i]).then(resolve);
                }
                ...
                if (motionGroupCount === 0) {
                    ...
                    this.createRenderer();
                    this.setupTextures();
                    if (gl) this.getRenderer().startUp(gl);
    ...
    private setupTextures(): void {
        ...
        if (this._state === LoadStep.LoadTexture) {
            ...
            for (
                ...
                if (
                    this._modelSetting.getTextureFileName(
                        modelTextureNumber,
                    ) === ''
                ) {
    ...
    public startMotion(
        ...
        no: number,
        priority: number,
        _onFinishedMotionHandler?: FinishedMotionCallback,
    ): CubismMotionQueueEntryHandle {
        if (!this._modelSetting) return;
        ...
        const onFinishedMotionHandler: FinishedMotionCallback = (self) => {
        ...
        if (motion == null) {
            fetch(`${this._modelHomeDir}${motionFileName}`)
                ...
                .then((arrayBuffer) => {
                    ...
                    let fadeTime: number =
                        this._modelSetting.getMotionFadeInTimeValue(group, no);
        ...
        if (motion != null) {
            ...
            if (this._debugMode) {
                LAppPal.printMessage(
                    `[APP]expression[${expressionId}] is null`,
                );
            }
        }
    }
    ...
    public hitTest(hitArenaName: string, x: number, y: number): boolean {
        if (!this._modelSetting) return false;
        ...
        for (let i = 0; i < count; i++) {
            if (this._modelSetting.getHitAreaName(i) === hitArenaName) {
                const drawId: CubismIdHandle =
                    this._modelSetting.getHitAreaId(i);
     ...
     */
    public preLoadMotionGroup(group: string): Promise<LAppModel> {
        return new Promise((resolve) => {
            if (!this._modelSetting) return;
            ...
            for (let i = 0; i < this._modelSetting.getMotionCount(group); i++) {
                ...
                fetch(`${this._modelHomeDir}${motionFileName}`)
                    ...
                    .then((arrayBuffer) => {
                        ...
                        if (tmpMotion != null) {
                            let fadeTime =
                                this._modelSetting.getMotionFadeInTimeValue(
                                    group,
                                    i,
                                );
                            if (fadeTime >= 0.0) {
                            ...
                            fadeTime =
                                this._modelSetting.getMotionFadeOutTimeValue(
                                    group,
                                    i,
                                );
                            if (fadeTime >= 0.0) {
